...
def populate_stats():
    ...
    r_item = requests.get(f"{app_config['eventstore']['url']}/get_item?timestamp={current_datetime}")
    r_brand = requests.get(f"{app_config['eventstore']['url']}/get_brand?timestamp={current_datetime}")
